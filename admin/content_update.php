...
if(is_uploaded_file($_FILES['image']['tmp_name'])) {
	...
	if(move_uploaded_file($sourcePath,$targetPath)) {
		...
		$result = $db_handle->runQuery("UPDATE `content` SET  `heading`='".$name."',`subheading`='".$sub."' ,`image`='".$targetPath."' where id='".$id."'");
    ...
    // echo "UPDATE `content` SET  `heading`='".$name."',`subheading`='".$sub."'  where id='".$id."'";exit;
$result1 = $db_handle->runQuery("UPDATE `content` SET  `heading`='".$name."',`subheading`='".$sub."'  where id='".$id."'");
echo '<script>alert(" Updated Successfully ");
window.location.href="content.php"; </script>';
