  ...
  $description=$_POST['description'];
    if(is_uploaded_file($_FILES['image']['tmp_name'])) {
      ...
      if(move_uploaded_file($sourcePath,$targetPath)){
        ...
        $result = $db_handle->runQuery("INSERT INTO `aboutcontent` (`id`,`image`,`heading`,`subheading`,`description`)values(NULL,'".$targetPath."','".$heading."','".$subheading."','".$description."')");
        echo '<script>alert(" Inserted Successfully ");
        window.location.href="aboutcontent.php"; </script>';
      }
      else{
        $result1 = $db_handle->runQuery("INSERT INTO `aboutcontent` (`id`,`heading`,`subheading`,`description`)values(NULL,'".$heading."','".$subheading."','".$description."')");
        echo '<script>alert(" Inserted Successfully ");
        window.location.href="aboutcontent.php"; </script>';
      }
